...
describe GameConfig do
  ...
  let(:player_config_double) { instance_double("PlayerConfig") }
  let(:game_config) { GameConfig.new(player_config_double) }
  ...
  describe "#set_up" do
    ...
    before(:each) do
      allow(player_config_double).to receive(:set_up).and_return(["Player 1", "Player 2"])
    end
